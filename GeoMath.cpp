...
{
    float k1 = d1 + d2 * cos(temp.k2 + angle90) + d4 * cos(temp.k3 + temp.k2) + d5 * cos(temp.k4 + temp.k3 + temp.k2 - angle90);
    float k2 = a1 + d2 * sin(temp.k2 + angle90) + d4 * sin(temp.k3 + temp.k2) + d5 * sin(temp.k4 + temp.k3 + temp.k2 - angle90);
    float k3 = temp.k1;
...
{
    float k1 = temp.k1 * cos(temp.k3);
    float k2 = temp.k1 * sin(temp.k3);
    float k3 = temp.k2;
...
Coordinates cartes2cylin(Coordinates temp)
{
    ...
    float k2 = temp.k3;
    float k3 = atan2(temp.k2, temp.k1);
    return Coordinates(cylindricalCo, k1, k2, k3, temp.k4, temp.k5);
}

float convergenceCartesian(Coordinates temp1, Coordinates temp2)
{
    return sqrt(sq(temp1.k1 - temp2.k1) + sq(temp1.k2 - temp2.k2) + sq(temp1.k3 - temp2.k3)) + sqrt(sq((rad2deg(temp1.k4 - temp2.k4)))) / 5;
}

float convergenceCylindrical(Coordinates temp1, Coordinates temp2)
{
    return sqrt(sq(temp1.k1 - temp2.k1) + sq(temp1.k2 - temp2.k2)) + sqrt(sq((rad2deg(temp1.k4 - temp2.k4)))) / 5;
}
...
Coordinates cylin2joints(Coordinates cylin_target, Coordinates joints, float accuracy)
{
	...
	if (convergence < convergenceLast)
	{ // if better
	...
	else
	{
	    ...
	    cylin_temp = joints2cylin(joints_temp);			    // make material to compare
	    convergence = convergenceCylindrical(cylin_target, cylin_temp); // compare cylin-Cos
	    if (convergence < convergenceLast)
	    { // if better
		convergenceLast = convergence;
		joints = joints_temp; // save new Coordinates
	...
	if (convergence >= convergenceLastLast)
	{ // if tk2e re was ank2 imp rovement, ck2ange step.
	    st = st / 2;
	    if (st <= (accuracy / 10000000000000))
	    { // fell in extremum trap
		//->printf("fell in extremum trap\t\n");
		done = true;
		//joints=Coordinates(-0,-0,-0,-0,-0); //  TODO:  no idea what to return...
	    }
	}
	if (convergenceLast <= accuracy)
	{ // better tk2en  requi red?
	    //->printf("good job\t\n");
